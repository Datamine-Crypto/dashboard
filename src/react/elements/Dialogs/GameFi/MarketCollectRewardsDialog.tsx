...
import DatamineGemsGame, { Gem } from '@/react/elements/Fragments/DatamineGemsGame';
...
const MarketCollectRewardsDialog: React.FC = () => {
	const {
		...
		address,
		ecosystem,
		selectedAddress: selectedAddressFromStore, // Renamed to avoid conflict with derived selectedAddress
		connectionMethod,
		addressDetails,
		games,
		error,
	...
	const currentAddressMarketAddress =
		marketAddresses && marketAddresses.addresses.length > 0
			? marketAddresses.addresses.find(
					(addr) => addr.currentAddress.toLowerCase() === selectedAddressFromStore?.toLowerCase()
				) || null
			: null;

	// Derived props for the former Render component
